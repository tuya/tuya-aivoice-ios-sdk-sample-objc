...
static NSString *const kPerfusionFillerChars = @"啊额呃嗯唉哎哦喔呵哈儿";
/// 行内 WER 达到该阈值即认为这一行没有被识别出来。
static const double kPerfusionMissedLineWER = 0.85;
/// 一个参考行最多允许由多少个识别分段合并而成（下限；实际值按识别段数与参考行数的比例上调）。
static const NSInteger kPerfusionMinMaxSpan = 8;

#pragma mark - 模型

@implementation PerfusionWERToken
@end

@implementation PerfusionWERResult
...
@implementation PerfusionWERLineResult
@end

#pragma mark - 计算

@implementation PerfusionWERCalculator
...
+ (NSCharacterSet *)fillerCharacterSet {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [NSCharacterSet characterSetWithCharactersInString:kPerfusionFillerChars];
    });
    return set;
}
...
+ (PerfusionWERResult *)evaluateReference:(nullable NSString *)reference
...
+ (PerfusionWERResult *)evaluateNormalizedReference:(NSString *)reference
    ...
    PerfusionWERResult *result = [[PerfusionWERResult alloc] init];
    ...
    if (n == 0 || m == 0) {
        NSMutableArray<PerfusionWERToken *> *alignment = [NSMutableArray array];
        for (NSString *token in refTokens) {
            PerfusionWERToken *item = [[PerfusionWERToken alloc] init];
            item.operation = PerfusionWEROperationDelete;
            item.referenceToken = token;
            [alignment addObject:item];
        }
        for (NSString *token in hypTokens) {
            PerfusionWERToken *item = [[PerfusionWERToken alloc] init];
            item.operation = PerfusionWEROperationInsert;
    ...
    NSMutableArray<PerfusionWERToken *> *reversed = [NSMutableArray array];
    NSUInteger i = n, j = m;
    ...
    while (i > 0 || j > 0) {
        ...
        PerfusionWERToken *item = [[PerfusionWERToken alloc] init];
        if (i > 0 && j > 0 && op == 0) {
            item.operation = PerfusionWEROperationEqual;
            item.referenceToken = refTokens[i - 1];
            item.hypothesisToken = hypTokens[j - 1];
            i--; j--;
        } else if (i > 0 && j > 0 && op == 1) {
            item.operation = PerfusionWEROperationSubstitute;
            item.referenceToken = refTokens[i - 1];
            item.hypothesisToken = hypTokens[j - 1];
            S++; i--; j--;
...
+ (NSArray<PerfusionWERLineResult *> *)evaluateReferenceLines:(NSArray<NSString *> *)referenceLines
    ...
    NSMutableArray<PerfusionWERLineResult *> *results = [NSMutableArray array];
    [referenceLines enumerateObjectsUsingBlock:^(NSString *refLine, NSUInteger idx, BOOL *stop) {
        ...
        PerfusionWERLineResult *line = [[PerfusionWERLineResult alloc] init];
        ...
        line.missed = line.result.wer >= kPerfusionMissedLineWER;
...
+ (NSArray<NSArray<NSNumber *> *> *)alignSegments:(NSArray<NSString *> *)segments
    ...
    NSInteger maxSpan = MAX(kPerfusionMinMaxSpan, (NSInteger)ceil(ratio) + 4);
