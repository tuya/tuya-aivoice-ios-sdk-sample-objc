...
/// 对齐后每个位置的操作类型。
typedef NS_ENUM(NSInteger, PerfusionWEROperation) {
    PerfusionWEROperationEqual = 0,      ///< 命中
    PerfusionWEROperationSubstitute,     ///< 替换（识别成了别的词）
    PerfusionWEROperationDelete,         ///< 删除（参考里有，识别结果里没有）
    PerfusionWEROperationInsert,         ///< 插入（识别结果里多出来的词）
};

/// 对齐序列中的一项，用于渲染逐词对比。
@interface PerfusionWERToken : NSObject
@property (nonatomic, assign) PerfusionWEROperation operation;
/// 参考词；插入时为 nil。
...
/// 一次 WER 计算的结果。
@interface PerfusionWERResult : NSObject
...
/// 逐词对齐序列。
@property (nonatomic, copy) NSArray<PerfusionWERToken *> *alignment;
...
/// 参考文本按行对比的结果。
@interface PerfusionWERLineResult : NSObject
/// 行号，从 1 开始。
...
@property (nonatomic, strong) PerfusionWERResult *result;
/// 该行几乎没有对应识别输出（行内 WER ≥ 0.85），视为漏识。
@property (nonatomic, assign) BOOL missed;
@end

@interface PerfusionWERCalculator : NSObject
