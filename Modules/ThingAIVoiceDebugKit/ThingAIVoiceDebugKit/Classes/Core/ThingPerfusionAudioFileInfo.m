...
/// 底层音频链路的期望参数。
static const NSUInteger kThingPerfusionExpectedSampleRate = 16000;
static const NSUInteger kThingPerfusionExpectedChannels = 1;
static const NSUInteger kThingPerfusionExpectedBits = 16;
/// WAV audioFormat：1 = 整型 PCM（底层唯一支持的），3 = IEEE float。
static const NSUInteger kThingPerfusionWaveFormatPCM = 1;
/// fmt chunk 通常在文件头部，读一小段即可，避免把大文件整个载入内存。
static const NSUInteger kThingPerfusionHeaderProbeLength = 4096;

@implementation ThingPerfusionAudioFileInfo

+ (ThingPerfusionAudioFileInfo *)infoAtPath:(nullable NSString *)path {
    ThingPerfusionAudioFileInfo *info = [[ThingPerfusionAudioFileInfo alloc] init];
    if (path.length == 0) return info;

    NSFileHandle *handle = [NSFileHandle fileHandleForReadingAtPath:path];
    if (!handle) return info;
    NSData *header = [handle readDataOfLength:kThingPerfusionHeaderProbeLength];
    [handle closeFile];
...
- (BOOL)isDecodable {
    ...
    return self.parsed && self.audioFormat == kThingPerfusionWaveFormatPCM;
}

- (BOOL)isRecommended {
    return self.isDecodable
        && self.sampleRate == kThingPerfusionExpectedSampleRate
        && self.channels == kThingPerfusionExpectedChannels
        && self.bitsPerSample == kThingPerfusionExpectedBits;
}
...
- (nullable NSString *)warning {
    ...
    if (self.sampleRate != kThingPerfusionExpectedSampleRate) {
        [issues addObject:[NSString stringWithFormat:@"采样率 %luHz（应为 %luHz）",
                           (unsigned long)self.sampleRate, (unsigned long)kThingPerfusionExpectedSampleRate]];
    }
    if (self.channels != kThingPerfusionExpectedChannels) {
        [issues addObject:[NSString stringWithFormat:@"%lu 声道（应为单声道）", (unsigned long)self.channels]];
    }
    if (self.bitsPerSample != kThingPerfusionExpectedBits) {
        [issues addObject:[NSString stringWithFormat:@"%lubit（应为 %lubit）",
                           (unsigned long)self.bitsPerSample, (unsigned long)kThingPerfusionExpectedBits]];
    }
